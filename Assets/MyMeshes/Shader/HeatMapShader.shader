  ...
  Properties
  {
    ...
    SubShader
    {
      ...
      Pass
      {
        ...
        float _Hits[3 * 32]; //passed in array of pointranges 3floats/point: world space x,y,z
        int _HitCount = 0;
        ...
        fixed4 frag(v2f i) : SV_Target
        {
          ...
          for (float j = 0.0; j < _HitCount; j++)
          {
            float3 work_pt = float3(_Hits[j * 3], _Hits[j * 3 + 1], _Hits[j * 3 + 2]);//float2(_Hits[i * 3], _Hits[i * 3 + 1]);
            float pt_intensity = 1.0;//= _Hits[i * 3 + 2];

            totalWeight += 0.5 * distsq(i.world_vertex, work_pt) * pt_intensity * _Strength * (1 + sin(_Time.y * _PulseSpeed));
          }
